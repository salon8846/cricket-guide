...
export const createSessionJsonlFileStore = ({
    directoryName,
    filePrefix,
    currentSessionId,
    maxBytes,
    maxFiles,
    createTruncatedEntry,
}) => {
    const directoryUri = `${LOG_ROOT_DIR}${directoryName}/`;
    ...
    const append = (entry) => runTask(async () => {
        ...
        if (info?.exists && Number(info.size ?? 0) > maxBytes) {
            if (!truncationRecorded && typeof createTruncatedEntry === 'function') {
                truncationRecorded = true;
                await writeFile(currentFileUri, toLine(createTruncatedEntry({ maxBytes })), { append: true });
            }
            return;
        }
