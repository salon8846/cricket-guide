            ...
            try {
                ...
                function logNativeWebViewUrl() {
                    if (nativeWebViewUrl && window.__nativeWebViewDebugConsoleUrl !== nativeWebViewUrl) {
                        ...
                        setTimeout(function() {
                            console.log('[WebViewPanel]', 'webview url:', nativeWebViewUrl);
                        }, 0);
                    }
                }
...
export function buildVConsoleDebugPanel(debugPanelScriptUrl, webViewUrl) {
    const scriptUrl = JSON.stringify(debugPanelScriptUrl);
    const loggedUrl = JSON.stringify(webViewUrl);
    return `
        (function() {
...
export function buildDebugPanelRemoval() {
    return `
        (function() {
            try {
